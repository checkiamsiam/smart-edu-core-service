...
const getSingleStudent: RequestHandler = catchAsyncErrors(
  async (req: Request, res: Response) => {
    ...
    const result: Partial<Student> | null =
      await studentService.getSingleStudent(id, req.queryFeatures);
    if (!result) {
...
const updateStudent: RequestHandler = catchAsyncErrors(
  async (req: Request, res: Response) => {
    ...
    const result: Partial<Student> | null = await studentService.updateStudent(
      id,
      updatePayload
    );
...
const deleteStudent: RequestHandler = catchAsyncErrors(
  async (req: Request, res: Response) => {
    const id: string = req.params.id;
...
const myCourses = catchAsyncErrors(async (req: Request, res: Response) => {
  const user = req.user;
  const result = await studentService.myCourses(
    user.userId,
...
const getMyCourseSchedules = catchAsyncErrors(
  async (req: Request, res: Response) => {
    const user = req.user;
